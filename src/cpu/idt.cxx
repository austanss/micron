...
void exception_handler(registers& regs)
{
	...
	term.write("$LIGHT_CYAN!CPU) ");
	printf("CR0=0x%x, CR2=0x%x, CR3=0x%x, CR4=0x%x,\n\t\t\tRIP=0x%x, RFLAGS=0x%x\n\n", regs.cr0, regs.cr2, regs.cr3, regs.cr4, regs.rip, regs.rflags);

	term.write("$LIGHT_CYAN!General) ");
	printf("RAX=0x%x, RBX=0x%x. RCX=0x%x, RDX=0x%x,\n\t\t\tRSI=0x%x, RDI=0x%x", regs.rax, regs.rbx, regs.rcx, regs.rdx, regs.rsi, regs.rdi);
